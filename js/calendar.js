...
class CalendarManager {
  ...
  async loadBookedSlots() {
    ...
    try {
      ...
      // Do NOT render an empty grid here: an all-"free" calendar is misleading —
      // the user could pick a date that is actually taken and only discover it at
      // the very end (submit fails). Show an explicit error + retry instead, which
      // also blocks date selection until availability really loads.
      this._renderError();
      if (typeof Toast !== 'undefined') {
        Toast.error('לא הצלחנו לטעון את זמינות התאריכים — בדקו את החיבור ונסו שוב', 7000);
      }
    } finally {
  ...
  /**
   * Render an error state in place of the calendar grid, with a retry button.
   * Used when availability can't be fetched — never show a misleading all-free grid.
   */
  _renderError() {
    ...
    // Keep the month label so the user still sees where they are.
    if (monthTitle) {
    ...
    // Preserve the weekday headers, drop the day cells, show an error panel.
    ...
    Object.assign(panel.style, {
      gridColumn: '1 / -1', textAlign: 'center', padding: '28px 16px',
      display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px',
    });
    panel.innerHTML = `
      <div style="font-size:30px">⚠️</div>
      <p style="font-weight:700;color:#e05c5c;margin:0">לא הצלחנו לטעון את זמינות התאריכים</p>
      <p style="opacity:.8;margin:0;font-size:14px">כדי למנוע בחירה בטעות של תאריך תפוס, אנא נסו שוב.</p>
    `;
  ...
  async _fetchMonth(year, month) {
    ...
    // Availability is read from booked_slots — a PII-free mirror of active
    // reservations — so the public site never needs read access to `bookings`.
  ...
  _buildDayCell({ day, date, dow, isPast, booked, isSelected }) {
    ...
    if (!isPast && dow !== 6) {
      ...
      // Keyboard accessibility: focusable button with a descriptive label.
      // Activation (Enter/Space) is handled by the global handler in app.js.
  ...
  /**
   * Build a descriptive aria-label for a clickable day cell, e.g.
   * "יום רביעי 10 בסיוון — בוקר פנוי, ערב תפוס".
   */
  _dayAriaLabel(date, dow, booked) {
    ...
    let avail;
    if (dow === 5) {
      avail = `אירוע סופ"ש ${booked.includes('weekend') ? 'תפוס' : 'פנוי'}`;
    } else {
      const am = booked.includes('morning') ? 'תפוס' : 'פנוי';
      const pm = booked.includes('evening') ? 'תפוס' : 'פנוי';
      avail = `בוקר ${am}, ערב ${pm}`;
    }
    return `יום ${dayName}, ${heb} — ${avail}`;
  }
  ...
  _showCoordinationMessage(container, date, dow, booked) {
    ...
    // Build skeleton without any dynamic text in innerHTML
    ...
    // Inject all dynamic content via textContent — immune to quotes / special chars
    div.querySelector('.date-header').textContent       = `יום ${dayName} | ${greg} | ${heb}`;
    div.querySelector('h4').textContent                 = title;
    div.querySelector('.status-message').textContent    = status;
    div.querySelector('.available-options').textContent = options;

    // Build WhatsApp button with addEventListener — greg/status are closed-over JS values,
    // never interpolated into an HTML attribute string, so סופ"ש can't break anything.
    const waBtn = document.createElement('button');
    waBtn.className = 'whatsapp-coordination-btn';
    waBtn.innerHTML = '<span class="whatsapp-icon">📱</span><strong>לתיאום האירוע</strong>';
